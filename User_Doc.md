...
| 字段     | 说明                                                                                              |
| -------- | ------------------------------------------------------------------------------------------------- |
| 主机     | SMTP 服务器地址，如 `smtp.resend.com`                                                             |
| 端口     | 决定加密方式：`465`/`2465`/`8465`/`443`/`588` 走隐式 SSL，其余（`587`/`25`/`2525`/`2587`）走 STARTTLS |
| 用户     | SMTP **认证用户名**，由服务商提供，**不一定是邮箱地址**                                            |
| 密码     | SMTP 密码 / API Key                                                                               |
| 发件邮箱 | 收件人看到的发件地址，**必须是服务商后台已验证的发件域或发件地址**；留空则回退为「用户」           |
| 发件人名 | 可选，发件人显示名，如 `UFI-TOOLS`                                                                |
| 发给     | 接收转发邮件的邮箱                                                                                |

> ⚠️ Resend / Mailjet / SMTP2GO / Brevo 等服务商的「认证用户名」与「发件邮箱」并不是同一个东西，需要分别填写：
>
> - **Resend**：主机 `smtp.resend.com`，端口 `465`（或 `2465`），用户固定填 `resend`，密码填 API Key，发件邮箱填已验证域名下的地址（如 `noreply@yourdomain.com`）
> - **Mailjet**：主机 `in-v3.mailjet.com`，端口 `587`，用户填 API Key，密码填 Secret Key，发件邮箱填已验证的 Sender 地址
> - **SMTP2GO**：主机 `mail.smtp2go.com`，端口 `2525`，用户/密码填后台 `Sending > SMTP Users` 里创建的凭据，发件邮箱填已验证的 Sender 地址
> - **Brevo**：主机 `smtp-relay.brevo.com`，端口 `587`，用户填账号邮箱或 SMTP 登录名，密码填 SMTP Key，发件邮箱填已验证的 Sender 地址
