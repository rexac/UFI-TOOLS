    ...
    let handlerStatusRender = async (flag = false) => {
        ...
        if (res) {
            ...
            let statusHtml_base = {
                ...
                battery: `${notNullOrundefinedOrIsShow(res, 'battery') && (res.battery_value != '' || res.battery_vol_percent != '') ? `<strong onclick="copyText(event)"  class="green">${res.battery_charging == "1" ? `${t('charging')}` : `${t('battery_level')}`}：${kano_parseSignalBar(res.battery+" %", 1, 100, 50, 10)}</strong>` : ''}`,
